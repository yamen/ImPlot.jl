...
function PlotShaded(x::AbstractArray{T,1}, y::AbstractArray{T,1}, y_ref::T;
    ...
    if eltype(x) == Float32
        LibCImPlot.PlotShadedFloatPtrFloatPtrInt(label, x, y, count, y_ref, offset,
                                                      stride * sizeof(Float32))
    elseif eltype(x) == Float64
        LibCImPlot.PlotShadeddoublePtrdoublePtrInt(label, x, y, count, y_ref,
                                                         offset, stride * sizeof(Float64))
    else
        x = convert.(Float32, x)
        y = convert.(Float32, y)
        y_ref = Float32(y_ref)
        LibCImPlot.PlotShadedFloatPtrFloatPtrInt(label, x, y, count, y_ref,
